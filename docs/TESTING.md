...
```bash
.venv/bin/scenelyr validate examples/order-platform.scene.json
.venv/bin/scenelyr compile examples/order-platform.scene.json artifacts
```

Inspect `artifacts/order-platform.svg` and `artifacts/order-platform.html`, then open the PPTX in
