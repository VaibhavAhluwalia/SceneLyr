...
def example_scene() -> SemanticScene:
    return SemanticScene.model_validate_json(Path("examples/order-platform.scene.json").read_text())


def test_layout_is_deterministic():
    first = layout_scene(example_scene()).to_dict()
    assert first == layout_scene(example_scene()).to_dict()
