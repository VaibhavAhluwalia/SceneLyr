...
<section class="card" style="padding:18px;margin-bottom:20px"><h2>Release validation</h2><p class="small">Run the documented image fixture matrix through the same importer used by MCP and this app.</p><button id="runValidation" class="button secondary" type="button">Run validation</button><span id="validationStatus" class="small" style="margin-left:10px"></span><a id="validationReport" class="button tiny" style="display:none;margin-left:8px" target="_blank">Open report</a></section>
...
document.getElementById('runValidation').addEventListener('click',async()=>{const status=document.getElementById('validationStatus'),report=document.getElementById('validationReport');status.textContent='Running…';report.style.display='none';try{const response=await fetch('/ui/release-validation',{method:'POST'}),body=await response.json();if(!response.ok)throw new Error(body.detail||'Validation failed');status.textContent=`${body.summary.passed}/${body.summary.total} fixtures passed`;report.href=body.reportUrl;report.style.display='inline-block'}catch(error){status.textContent=error.message}});
