...
function select(id) {
 state.selected=id; remember('selection.'+state.scene.id,id); syncSelection();
}
...
function renderInspector() {
 ...
 if(node) {
  ...
  html=`<span class="eyebrow">OBJECT / ${escape(m.shape||node.kind)}</span><h2 class="inspector-title">${escape(node.label)}</h2><span class="muted">${escape(node.id)}</span><label class="inspector-label" for="node-label">Label</label><input id="node-label" value="${escape(node.label)}" maxlength="500"><button class="primary" id="rename-node">Preview Rename</button><div class="inspector-section"><h3>Source Evidence</h3>${node.cropUrl?`<img class="source-crop" src="${escape(node.cropUrl)}?v=${data.revision.slice(0,8)}" alt="Original source crop of ${escape(node.label)}">`:'<p class="muted">No source crop. This object may have been added semantically.</p>'}<div class="evidence-grid">${evidence('Text confidence',m.ocrConfidence==null?null:Math.round(m.ocrConfidence*100)+'%')}${evidence('Label status',m.labelStatus||'Semantic')}${evidence('Detection',m.method||'User added')}${evidence('Source shape',m.shape||'Not detected')}</div>${needsReview(node)?'<div class="review-card"><strong>! Review Required</strong>Compare this label and shape with the original image.</div>':''}<div class="asset-links"><a href="${escape(node.recordUrl)}" target="_blank" rel="noopener">Object JSON</a><a href="${escape(node.layerUrl)}" target="_blank" rel="noopener">SVG Layer</a></div></div><div class="inspector-section"><h3>Semantic Actions</h3><button class="action" data-command="${escape('remove '+quote(node.id))}">Remove Object…</button></div><details><summary>Detection Details</summary><pre class="evidence-json">${escape(JSON.stringify(m,null,2))}</pre></details>`;
 } else if(edge) {
...
$('canvas').addEventListener('click',e=>{const element=e.target.closest('[data-scene-id],[data-edge-id]');if(element)select(element.dataset.sceneId||element.dataset.edgeId);});
